...
export class ValueComponent implements OnInit {
  ...
  onSubmit(form: NgForm) {
    if (form.value._id == "") {
      this.postService.postValue(form.value).subscribe(
        (res) => {
          // this.showSucessMessage = true;
          this.refreshpostList();
          this.showNotification("Valeur est ajouté avec succès");
          this.resetForm(form);
      ...
      this.postService.putPost(form.value).subscribe(
        (res) => {
          this.showSucessMessage = true;
          this.refreshpostList();

          this.showNotification("Mise a jour de valeur se fait avec succès");
  ...
  onDeleteshowSwal(_id: string) {
    Swal.fire({
      title: "Vous êtes sur de supprimer ce valeur?",
      text: "",
      type: "warning",
      ...
      if (result.value) {
        ...
        Swal.fire({
          title: "Supprimer!",
          text: "Valeur est supprimé avec succès.",
          type: "success",
          confirmButtonClass: "btn btn-success",
          buttonsStyling: false,
        });
